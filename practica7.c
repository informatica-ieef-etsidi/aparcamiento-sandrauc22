...
void main() {
	...
	char fichero[20] = "aparcamiento.txt";
	int nplazas;
	...
	leerFichero(fichero, plaza, &nplazas);
		...
		if (usuValido == 0) {
			while (salida2 == 0) {
				...
				case 'S':
					salida = 1;
					salida2 = 1;
					salvarFichero(fichero, plaza, nplazas);
					break;
...
void leerFichero(char nombreFichero[], struct plaza aparcamiento[], int * dim) {
	...
	i = 0;
	while (fscanf(fichero, "%d %c %s", &aparcamiento[i].estado, &aparcamiento[i].tipo, &aparcamiento[i].matricula, 8) != EOF) {
		i++;
	}
	*dim = i;
	fclose(fichero);
